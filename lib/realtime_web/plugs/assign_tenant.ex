defmodule RealtimeWeb.Plugs.AssignTenant do
  ...
  def call(%Plug.Conn{host: host} = conn, _opts) do
    with {:ok, external_id} <- get_external_id(host),
      ...
      tenant =
        tenant
        |> tap(&GenCounter.new({:limit, :plug, &1.external_id}))
        |> tap(&RateCounter.new({:limit, :plug, &1.external_id}, idle_shutdown: :infinity))
        |> tap(&GenCounter.add({:limit, :plug, &1.external_id}))
