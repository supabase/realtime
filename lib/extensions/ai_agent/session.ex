defmodule Extensions.AiAgent.Session do
  ...
  def init(opts) do
    ...
    with {:ok, adapter} <- resolve_adapter(settings) do
      ...
      init_result(state, client_session_id)
    else
  ...
  defp init_result(state, nil), do: {:ok, state}
  defp init_result(state, _session_id), do: {:ok, state, {:continue, :load_history}}

  @impl true
  def handle_continue(:load_history, state) do
    prior = load_history(state.tenant_id, state.tenant_topic, state.session_id)
    # Messages are stored newest-first and reversed once in start_stream.
    # Prepend reversed history so Enum.reverse produces [system_msg | hist_asc...].
    state = %{state | messages: Enum.reverse(prior) ++ state.messages}
    {:noreply, state}
  end
  ...
  def handle_cast(:emit_session_started, state) do
  ...
  def handle_info({:ai_event, %Event{type: :tool_call_done} = event}, state) do
    ...
    broadcast_event(state, event)
    {:noreply, %{state | messages: [assistant_tool_call | state.messages]}}
  end
  ...
  defp stream_with_message(%{"role" => "user"} = msg, _body, state) do
    ...
    {:noreply, start_stream(%{state | messages: [msg | state.messages]})}
  end
