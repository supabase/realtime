    ...
    case RateCounter.get(rate_counter_args) do
      ...
      _ ->
        ...
        for row <- rows,
          ...
          nodes =
            MapSet.new(change.subscription_ids, fn subscription_id ->
              [{_, node}] = :ets.lookup(subscribers_nodes_table, subscription_id)
              node
            end)

          for node <- nodes do
            RealtimeWeb.TenantBroadcaster.pubsub_direct_broadcast(
              node,
              tenant_id,
              topic,
              change,
