defmodule Realtime.GenCounter do
  @moduledoc """
  A generic counter interface for any Erlang term.
  """
  ...
  def handle_call(:new, _from, state) do
    {:ok, index} = find_index(self())
    ref = :counters.new(index, [:write_concurrency])
  ...
  defp find_index(pid) do
    children =
      DynamicSupervisor.which_children(GenCounter.DynamicSupervisor)
      |> Enum.with_index(1)
      |> Enum.find(fn {{:undefined, worker_pid, :worker, _}, _index} -> pid == worker_pid end)

    case children do
      {_worker, index} -> {:ok, index}
      _error -> {:error, :child_not_found}
    end
  end
