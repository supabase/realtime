defmodule Realtime.PromEx.Plugins.Channels do
  ...
  @sessions_event_cluster [:prom_ex, :plugin, :realtime, :sessions_cluster]
  @topics_event_cluster [:prom_ex, :plugin, :realtime, :topics_cluster]
  @disconnected_cluster_event [:prom_ex, :plugin, :realtime, :disconnected_cluster]
  @joined_cluster_event [:prom_ex, :plugin, :realtime, :joined_cluster]
  @disconnected_event [:prom_ex, :plugin, :realtime, :disconnected]
  @msg_sent_event [:prom_ex, :plugin, :realtime, :msg_sent]
  @msg_sent_cluster_event [:prom_ex, :plugin, :realtime, :msg_sent_cluster]
  ...
  def local_online() do
    Registry.count(Realtime.Registry.Unique)
  end
  ...
  def local_topics() do
    Registry.count_match(Realtime.Registry, "topics", {:_, :_, :_})
  end
  ...
  def local_msg_sent() do
    config = Core.Registry.config(Realtime.PromEx.Metrics)
    ts = Core.Aggregator.get_time_series(config.aggregates_table_id)

    case ts[[:realtime, :realtime_channel, :msg_sent]] do
  ...
  def local_joined() do
    ...
    case ts[[:realtime, :prom_ex, :phoenix, :channel, :joined, :total]] do
      [{_, count}] when is_integer(count) -> count
      _ -> 0
    end
  end
  ...
  def local_disconnected() do
    ...
    case ts[[:realtime, :realtime_channel, :disconnected]] do
      [{_, count}] when is_integer(count) -> count
      _ -> 0
    end
  end
