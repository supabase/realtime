defmodule Realtime.GenRpcPubSub do
  ...
  def broadcast(adapter_name, topic, message, dispatcher) do
    ...
    if Application.get_env(:realtime, :regional_broadcasting, false) do
      ...
      # broadcast to all nodes in the region
      GenRpc.abcast(Nodes.region_nodes(my_region), worker, Worker.forward_to_local(topic, message, dispatcher),
        key: worker
      )

      # send a message to a random node in each region to forward to the rest of the region
      other_region_nodes = random_node_from_other_regions(my_region, worker)
  ...
  defp random_node_from_other_regions(my_region, key) do
    Enum.flat_map(Nodes.all_node_regions(), fn
      ^my_region ->
        []

      region ->
        case Nodes.random_node_from_region(region, key) do
          {:ok, random_node} -> [random_node]
          _ -> []
        end
    end)
  end
...
defmodule Realtime.GenRpcPubSub.Worker do
  ...
  def init({pubsub, worker}) do
    ...
    Process.flag(:fullsweep_after, 50)
    {:ok, {pubsub, worker}}
  end
