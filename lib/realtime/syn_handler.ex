defmodule Realtime.SynHandler do
  ...
  def resolve_registry_conflict(mod, name, {pid1, _meta1, _time1}, {pid2, _meta2, _time2}) do
    ...
    if node(pid_to_stop) == node() do
      log(
        "Resolving conflict on scope #{inspect(mod)} for name #{inspect(name)} {#{inspect(pid1)}} vs {#{inspect(pid2)}}, stop local process: #{inspect(pid_to_stop)}"
      )

      stop(pid_to_stop)
    else
      log(
        "Resolving conflict on scope #{inspect(mod)} for name #{inspect(name)} {#{inspect(pid1)}} vs {#{inspect(pid2)}}, remote process will be stopped: #{inspect(pid_to_stop)}"
      )
    end
