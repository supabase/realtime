defmodule RealtimeWeb.ExecutionControllerTest do
  ...
  describe "index" do
    test "list all workflow's executions", %{conn: conn} do
      ...
      conn =
        post(
          conn,
          Routes.workflow_execution_path(conn, :create, workflow_id),
          execution_fixture()
        )
  ...
  describe "show" do
    test "returns 404 if the execution does not exist", %{conn: conn} do
      ...
      conn =
        get(
          conn,
          Routes.workflow_execution_path(
            conn,
            :show,
            workflow_id,
            "da56c89b-f811-4bdc-b009-ee62e23712b4"
          )
        )
  ...
  describe "delete" do
    test "returns the newly deleted execution", %{conn: conn} do
      ...
      conn =
        delete(conn, Routes.workflow_execution_path(conn, :delete, workflow_id, execution_id))
