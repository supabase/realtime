...
if Realtime.Env.get_boolean("WAIT_MARGIN_REPORT", false) do
  ...
  :telemetry.attach(
    ...
    fn _event, %{duration: duration, evaluations: evaluations}, meta, _config ->
      # `refute_eventually` and `assert_always` pass *by* timing out, so their timeouts are not
      # news. WaitForIt tags them, which is the only way to tell them apart from a real one.
      expected_timeout? = meta.wait_context in [%{construct: :refute_eventually}, %{construct: :assert_always}]
      ...
      report? = not expected_timeout? and (meta.result == :timeout or over_budget?)
