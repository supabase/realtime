defmodule Extensions.AiAgent.Adapter.AnthropicMessagesTest do
  ...
  describe "stream/3" do
    ...
    test "emits error event on HTTP 5xx" do
      caller = self()

      stub(Finch, :stream, fn _req, _name, acc, callback, _opts ->
        acc = callback.({:status, 529}, acc)
        {:ok, acc}
      end)

      assert :ok = AnthropicMessages.stream(@settings, @messages, caller)
      assert_receive {:ai_event, %Event{type: :error, payload: %{reason: "HTTP 529"}}}
    end

    test "skips malformed SSE lines without crashing" do
      caller = self()

      stub(Finch, :stream, fn _req, _name, acc, callback, _opts ->
        bad_data = "data: not-valid-json\n\n"
        ...
        acc = callback.({:status, 200}, acc)
        acc = callback.({:data, bad_data <> valid}, acc)
        {:ok, acc}
      end)

      assert :ok = AnthropicMessages.stream(@settings, @messages, caller)
      assert_receive {:ai_event, %Event{type: :done}}
      refute_receive {:ai_event, %Event{type: :error}}, 50
    end
  end
  ...
  describe "system_prompt" do
    ...
    test "sends system_prompt as top-level system field" do
      caller = self()
      settings = Map.put(@settings, "system_prompt", "You are a helpful assistant.")

      stub(Finch, :stream, fn req, _name, acc, callback, _opts ->
        send(caller, {:captured_body, Jason.decode!(req.body)})
        acc = callback.({:status, 200}, acc)
        acc = callback.({:data, minimal_sse()}, acc)
        {:ok, acc}
      end)
    ...
    test "omits system field when system_prompt is absent" do
      caller = self()
    ...
    test "filters system-role messages from the messages array" do
      caller = self()
      settings = Map.put(@settings, "system_prompt", "Be helpful.")
