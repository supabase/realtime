defmodule Extensions.AiAgent.Adapter.ChatCompletionsTest do
  ...
  describe "stream/3" do
    ...
    test "emits error event on HTTP 5xx" do
      ...
      stub(Finch, :stream, fn _req, _name, acc, callback, _opts ->
        acc = callback.({:status, 500}, acc)
        {:ok, acc}
      end)

      assert :ok = ChatCompletions.stream(@settings, @messages, caller)
      assert_receive {:ai_event, %Event{type: :error, payload: %{reason: "HTTP 500"}}}
    end
    ...
    test "skips malformed SSE lines without crashing" do
      ...
      stub(Finch, :stream, fn _req, _name, acc, callback, _opts ->
        bad_data = "data: {broken json\n\n"
        finish = sse(%{"choices" => [%{"delta" => %{}, "finish_reason" => "stop"}]})

        acc = callback.({:status, 200}, acc)
        acc = callback.({:data, bad_data <> finish <> done_chunk()}, acc)
        {:ok, acc}
      end)
