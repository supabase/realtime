defmodule Extensions.AiAgent.SessionTest do
  ...
  defp start_session(overrides \\ []) do
    ...
    GenServer.cast(pid, :emit_session_started)
    pid
  end
  ...
  describe "start_link/1" do
    ...
    test "emits session_started event on init" do
      start_session()
      assert_receive %Phoenix.Socket.Broadcast{event: "ai_event", payload: %{"event" => "agent_session_started"}}, 500
    end
  end
  ...
  describe "handle_input/2 with text" do
    test "sends text_delta directly to channel and done via PubSub" do
      ...
      assert_receive %Phoenix.Socket.Broadcast{
                       event: "ai_event",
                       payload: %{"event" => "agent_text_delta", "payload" => %{delta: "Hello"}}
                     },
                     1000

      assert_receive %Phoenix.Socket.Broadcast{event: "ai_event", payload: %{"event" => "agent_done"}}, 1000
    end
    ...
    test "rejects text larger than 64KB" do
      ...
      assert_receive %Phoenix.Socket.Broadcast{
                       event: "ai_event",
                       payload: %{"event" => "agent_error", "payload" => %{reason: "input_too_large"}}
                     },
                     500
    end
  end
  ...
  describe "cancel/1" do
    test "cancels an in-flight stream" do
      ...
      refute_receive %Phoenix.Socket.Broadcast{event: "ai_event", payload: %{"event" => "agent_done"}}, 200
    end
  end
  ...
  describe "adapter crash isolation" do
    test "broadcasts error event when adapter raises instead of crashing session" do
      ...
      assert_receive %Phoenix.Socket.Broadcast{event: "ai_event", payload: %{"event" => "agent_error"}}, 1000
      assert Process.alive?(pid)
    end
  end
  ...
  describe "system_prompt initialization" do
    test "includes system message first in request when system_prompt is set" do
      caller = self()
      settings = Map.put(@settings, "system_prompt", "You are a pirate.")

      stub(Finch, :stream, fn req, _name, acc, callback, _opts ->
        send(caller, {:captured_body, Jason.decode!(req.body)})
        acc = callback.({:status, 200}, acc)
        acc = callback.({:data, sse_done()}, acc)
        {:ok, acc}
      end)

      pid = start_session(settings: settings)
      assert_receive %Phoenix.Socket.Broadcast{event: "ai_event", payload: %{"event" => "agent_session_started"}}, 500

      Session.handle_input(pid, %{"text" => "ahoy"})

      assert_receive {:captured_body, body}, 1000
      assert [%{"role" => "system", "content" => "You are a pirate."} | _] = body["messages"]
    end

    test "sends no system message when system_prompt is absent" do
      caller = self()
