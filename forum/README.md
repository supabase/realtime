...
* If a source node is removed from the cluster, the routers still listing it drop its rows in their peer-`:DOWN` handler (`match_delete` on that node) — a dead source can never flush its own vacancies, so nothing else would.
* The source node's periodic vacant flush re-sends queued vacancies to the *current* router, so a vacancy that was never delivered (or was delivered to a node that has since stopped being the router) is eventually applied where it matters. Because each shard's claim state is **crash-durable** (see [Coordinator or shard crash](#coordinator-or-shard-crash-for-other-reasons)), a shard restart no longer forgets queued/cooldown vacancies — the flush resumes retracting them, including for groups routed to a *remote* router that the local node would otherwise have no record of.
