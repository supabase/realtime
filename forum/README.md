...
**Draining is outbound-only.** While leaving, the node suppresses every *outbound self-assertion* (heartbeat announce, re-discovery, discover-ack, nodeup-discovery) so no peer can re-pair it after it announced leaving. Every *inbound coordination RPC* (`:occupied`, `:vacant_batch`, snapshot/delta/transition applies, `:rebalance_marker`) keeps being serviced through the whole drain + settle window — the node stays a responsive router right up to exit; it just stops advertising itself. `join/3` on a draining node returns `{:error, :draining}` so a stray join fails loudly instead of creating an unroutable member.

**A draining router floods; it never decides.** At drain start the leaver publishes `:rebalancing` and stays there (`update_status` is a no-op while leaving), so `targets/3` on it returns `{:error, :flood}` for every sender and its own `router/2` returns the full member list. This is not caution for its own sake. Peers evict the leaver one at a time, and from the *first* eviction its occupancy table is no longer maintained: a peer that has evicted it sends its new first-member claims to the newly-elected router, never to the leaver. A peer that has *not* yet processed the leave still routes those same groups to the leaver and asks it under the old view hash — which the leaver would match, since it keeps its old view, and answer from a table that is missing the other peer's brand-new members. That is a silent miss the crash path never has: a dead router fails the call and the sender floods. The settle window's job (letting broadcasts already routed here fan out) is served by flooding just as well.
...
`drain/2` closes that window while the node is **still alive**. The leaver does *not* rebalance itself: it keeps its own full ring and view, and lets each **peer** rebalance *it* out of that peer's ring. That holds even when a peer dies *under* it mid-drain (a concurrent rolling restart, or two nodes draining at once): `depart_peer/3` on a leaver forgets the peer but skips the rebalance, and any in-flight growth prepare is dropped at drain start. A rebalance from a leaver would dispatch fresh snapshots/deltas, stamped *above* the departure watermark (below) that peers parked for it, to peers that already evicted it — each would pass their guard and resurrect the leaver as a phantom until the reap after its death — and a failed one would crash the coordinator, whose restart reopens the join gate and re-pairs a node that is on its way out. Two messages carry the entire protocol:
...
**`drain` is terminal.** It leaves the node draining with its heartbeat off and its status `:rebalancing`, so a node that calls `drain` but does not actually die (e.g. an aborted deploy) is stranded until it restarts. Call it only when truly shutting down. A second `drain/2` while one is in flight (or after one completed) returns `{:error, :already_draining}` and leaves the first untouched; it would otherwise orphan the first caller and re-broadcast a leave that peers who already evicted us would never ack.
