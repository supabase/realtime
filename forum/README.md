...
* A peer's coordinator `:DOWN` (crash or disconnect): the peer is removed and `recompute_members` runs. Before rebalancing, the `:DOWN` handler also wipes every occupancy row keyed by the departed node, since a dead source can never flush its vacancies and nothing else would clean them — **unless another live peer for that same node is already registered**, in which case the wipe is skipped entirely: `peers` is keyed by pid, not node, and a peer that restarts *in place* can have its fresh discovery re-register a new pid for the same node before this handler ever runs for the old pid's `:DOWN` (discovery travels the adapter channel, `:DOWN` travels the monitor channel, and nothing orders the two). Without the guard, the dead pid's `:DOWN` would destroy occupancy/`member_views`/`applied_snapshot_seq` data the new, still-live incarnation already delivered and had acknowledged — permanently, since membership hasn't changed and nothing triggers a re-announce. (`test/forum/muster_distributed_test.exs` kills a whole node and asserts the departure path end-to-end: groups the dead node routed move onto the survivors and are re-announced to their new routers, no survivor's occupancy table still lists the dead node as a source, and the remaining cluster re-converges to `:ready`. A separate test drives the fresher-incarnation race deterministically and asserts the newer incarnation's delivered row survives the old pid's `:DOWN`.)
...
* If the source node leaves the cluster, the router's `:DOWN` handler clears all entries keyed by that node — unless another live peer pid for that same node is already registered (a fresher incarnation's discovery outracing the dead pid's `:DOWN`), in which case the clear is skipped so the newer incarnation's data isn't destroyed.
...
* If a source node is removed from the cluster, the routers still listing it drop its rows in their peer-`:DOWN` handler (`match_delete` on that node), since a dead source can never flush its own vacancies and nothing else would — unless another live peer pid for that node is already registered, in which case the drop is skipped (see "Vacant-time RPC failure" above).
