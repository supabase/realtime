...
Rows the guard skips are harmless — a non-router's rows are never consulted, so they cost only memory — and they are re-judged on the `:ready` transition, by which point every member has announced agreement with our view and the sweep can collect everything genuinely stale. (`test/forum/muster_distributed_test.exs` drives a router through both a partial-view and a stale-view sweep with a real source's row in place and asserts the guard spares it.)
