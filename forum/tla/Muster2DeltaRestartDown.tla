------------------------- MODULE Muster2DeltaRestartDown -----------------------
(* EXPERIMENT variant of Muster2DeltaRestart: models the peer-side coordinator-  *)
(* pid :DOWN that the base model omitted. In the real code peers MONITOR the      *)
(* restarting node's coordinator PID (see scope.ex :DOWN handler ~L815-840, which *)
(* explicitly handles "a peer that restarts in place" / "the OLD pid's DOWN"), so *)
(* a coordinator restart delivers a :DOWN to every peer, wiping that node's        *)
(* member_views agreement there. Here Restart SYNCHRONOUSLY blanks mv[p][n] for   *)
(* every peer p (the most generous / instant version of that :DOWN). If           *)
(* NoMissedDelivery still fails, the stale-ready window is robustly real; if it    *)
(* now HOLDS, the base-model violation was caused purely by omitting the :DOWN     *)
(* and the residual real window is only the async :DOWN-delivery latency.          *)
(*****************************************************************************)
...
Restart(n) ==
  ...
  \* member_views wiped on n itself AND (peer-side :DOWN) n's agreement blanked
  \* on every peer p -- the synchronous, most-generous model of the coordinator-pid
  \* :DOWN every peer receives when n's coordinator restarts.
  /\ mv'    = [p \in Nodes |->
  ...
  /\ occ' = [occ EXCEPT ![n] = [g \in Groups |-> [s \in Nodes |->
        ...
        THEN [present |-> TRUE, seq |-> Bump(n)]
        ELSE occ[n][g][s]]]]
  \* FIFO faithfulness: the coordinator-pid :DOWN each peer receives is ordered
  \* AFTER every message the OLD incarnation sent (Erlang monitor+dist ordering),
  \* so no old-incarnation message survives to re-establish a stale agreement past
  \* the :DOWN. Every in-flight msg from n right now is from the old incarnation
  \* (the new one has not run yet), so drop them -- the unordered `msgs` set would
  \* otherwise let a stale marker land AFTER the wipe and re-ready a peer, a
  \* reordering real FIFO forbids (see TLA_FINDINGS caveat 5).
